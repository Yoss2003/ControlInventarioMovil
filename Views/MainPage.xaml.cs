...
public partial class MainPage : ContentPage
{
    ...
    private async void OnBotonOrbitalTapped(int index)
    {
        ...
        if (Math.Abs(distancia) > 0.01)
        {
            ...
            while (stopwatch.Elapsed.TotalMilliseconds < duracionVelozMs)
            {
                double progresoLineal = Math.Min(stopwatch.Elapsed.TotalMilliseconds / duracionVelozMs, 1.0);

                // Fórmula de Aceleración Smooth (Cubic In-Out): Arranca lento, va rápido en el centro, frena suave
                double progresoConEasing = progresoLineal < 0.5
                    ? 4 * Math.Pow(progresoLineal, 3)
                    : 1 - Math.Pow(-2 * progresoLineal + 2, 3) / 2;
        ...
        await DisplayAlert("Navegación", $"Abriendo vista de: {_textosOrbitales[index].Text}", "OK");
